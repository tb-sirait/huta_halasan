...
export default function BatakCalendarSEO({ year, month }) {
  ...
  const jsonLdDataset = {
    ...
    inLanguage: ["id", "bbc"],
    keywords: [
      "Kalender Batak", "Batak calendar", "Parmalim",
      "Hari Batak", "Sipaha Sada", "Hurung", "Lamadu", "Sipaha Lima",
      "konversi kalender", "Mangan Napaet", "Robu", "Ari Pameleon Bolon",
  ...
  const metaKeywords = [
    ...
    bulanBatakList,
    ...BATAK_HARI,
    "Parmalim", "Sipaha Sada", "konversi tanggal Batak", "Batak calendar converter",
    "Mangan Napaet", "Robu", "Ari Pameleon Bolon", "Ari Hatutubu ni Tuhan Simarimbulubosi",
  ].join(", ");
  ...
  return (
    <>
      ...
      <Helmet>
        ...
        <link rel="canonical"    href={`https://hutahalasan.com/kalender/${year}/${month}`} />

        <meta property="og:type"        content="website" />
        <meta property="og:title"       content={`Kalender Batak ${monthLabel} ${year}`} />
        <meta property="og:description" content={metaDesc} />
        <meta property="og:url"         content={`https://hutahalasan.com/kalender/${year}/${month}`} />
        <meta property="og:site_name"   content="Parmalim Bale Pasogit Huta Halasan" />
      ...
      >
        ...
        <p>
          ...
          Mangan Napaet jatuh pada tanggal {mn1} dan {mn2} bulan {bulanTerakhir}.
        </p>
        ...
        <table summary={`Konversi tanggal Masehi ke Kalender Batak — ${monthLabel} ${year}`}>
          ...
          <tbody>
            {days.map(({ day, date, hari, tanggal, bulan, parmalim }) => (
              <tr key={day}>
                <td><time dateTime={date.toISOString().split("T")[0]}>{day} {monthLabel} {year}</time></td>
                <td>{hari}</td>
                <td>{tanggal}</td>
                <td>{bulan}</td>
                <td>{parmalim || "—"}</td>
        ...
        <section aria-label="Aturan Mangan Napaet tahun ini">
          ...
          <p>
            Pada tahun {year}, bulan terakhir kalender Batak adalah <strong>{bulanTerakhir}</strong> dengan
            panjang <strong>{panjangBulanTerakhir} hari</strong>.
            Oleh karena itu, Mangan Napaet jatuh pada tanggal{" "}
            <strong>{mn1}</strong> dan <strong>{mn2}</strong> bulan {bulanTerakhir}.
          </p>
        </section>

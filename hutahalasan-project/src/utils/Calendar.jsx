...
const Calendar = () => {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    const parmalim = getParmalimEvent(date);
    if (parmalim) parmalimThisMonth.push({ d, parmalim, batak: getBatakInfo(date) });
  }
      ...
      >
        ...
        {(parmalim || hasNatl) && (
          <div className="day-dots">
            ...
            {hasNatl  && <span className={`dot national-dot${bothDots ? " nudge" : ""}`} title={currentEvents[d]} />}
          </div>
        )}
    ...
    <div className="calendar-container">
        ...
        <div className="calendar-title-block">
          ...
          <p className="calendar-batak-sub">
            {batakMonths.map((bm, i) => (
              <span key={bm}>
                {i > 0 && " · "}
                <span className={bm === "Lamadu" ? "calendar-lamadu-badge" : undefined}>{bm}</span>
              </span>
            ))}
          </p>
        </div>
            ...
            <ul className="calendar-events-list">
              {parmalimThisMonth.map(({ d, parmalim, batak }) => (
                <li key={`p-${d}`} className="calendar-event-item">
                  <span className="event-date parmalim-event-date">{d} {MONTH_NAMES[month]}</span>
                  <span className="event-name parmalim-event-name">{parmalim}</span>
                  <span className="event-batak-info">{batak.hari} · {batak.bulan}</span>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div className="footer-section">
            ...
            <ul className="calendar-events-list">
              {Object.keys(currentEvents)
                .sort((a, b) => a - b)
                .map((d) => {
                  const batak = getBatakInfo(new Date(year, month, parseInt(d)));
                  return (
                    <li key={d} className="calendar-event-item">
                      <span className="event-date">{d} {MONTH_NAMES[month]}</span>
      ...
      {tooltip.visible && tooltip.data && (
        ...
        >
          ...
          <div className="tooltip-line">Tanggal Batak: {tooltip.data.batak.tanggal}</div>

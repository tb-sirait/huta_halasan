...
export const BATAK_MONTH_TRANSITIONS = [
  ...
  { date: new Date(2025, 1,  28), bulan: "Lamadu",         tanggal: 1 }, // bulan ke-13, 30 hari
  ...
  { date: new Date(2026, 1,  17), bulan: "Hurung",         tanggal: 1 }, // 30 hari
...
export function getBatakInfo(date) {
  ...
  const diffDays = Math.round(
    (d - new Date(lastEntry.date.getFullYear(), lastEntry.date.getMonth(), lastEntry.date.getDate())) /
      86400000,
  );
 ...
 *
 * 4. Ari Pameleon Bolon — tgl 11 & 12 bulan Sipaha Lima
 *    (hari Muda Ni Mangadop tgl 11, dan Boraspati Ni Tangkup tgl 12)
 *    Perayaan berlangsung 2 hari.
 *
 * @param {Date} date
 * @returns {string|null}
 */
export function getParmalimEvent(date) {
  ...
  // ── 2. Robu (Tahun Baru Batak) ──────────────────────────────────────────
  if (bulan === "Sipaha Sada" && tanggal === 1) {
    return "Robu";
  }

  // ── 3. Ari Hatutubu ni Tuhan Simarimbulubosi ────────────────────────────
  if (bulan === "Sipaha Sada" && (tanggal === 2 || tanggal === 3)) {
    return "Ari Hatutubu ni Tuhan Simarimbulubosi";
  }

  // ── 4. Ari Pameleon Bolon ───────────────────────────────────────────────
  // Berlangsung 2 hari:
  //   Tgl 11 Sipaha Lima = Muda Ni Mangadop
  //   Tgl 12 Sipaha Lima = Boraspati Ni Tangkup
  if (bulan === "Sipaha Lima" && (tanggal === 11 || tanggal === 12)) {

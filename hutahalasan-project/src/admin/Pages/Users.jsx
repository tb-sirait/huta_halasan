...
import { formatDate, getInitials, subroleColor, truncate } from '../utils/helpers';
import '../styles/user.css';

const PER_PAGE     = 15;
const ADMIN_ROLES  = ['Validator', 'Jurnalis', 'Pengembang', 'Manajer'];
const ROLE_COLORS  = {
  Pengembang: '#2980b9', Manajer: '#27ae60',
  Validator:  '#d4561c', Jurnalis: '#8b6f47', User: '#a0a0a0',
};
...
export const Users = () => {
  ...
  useEffect(() => {
    ...
    searchTimer.current = setTimeout(() => { setPage(1); fetchList(1); }, 350);
  ...
  return (
    <div>
      ...
      <div className="user-toolbar">
        ...
        <button className="btn btn-ghost btn-sm" onClick={() => fetchList(page)}>↻ Refresh</button>
      </div>
              ...
              Array.from({ length: 6 }).map((_, i) => (
                <tr key={i}>
                  {Array.from({ length: 6 }).map((_, j) => (
                    <td key={j}>
                      <div className="dashboard-skeleton" style={{ height: 14, width: '80%' }} />
                    </td>
                  ))}
                </tr>
              ))
              ...
              <tr key={u.id_user}>
                <td>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div className="user-avatar-cell">{getInitials(u.nama_user || u.email)}</div>
                    <div>
                      <div style={{ fontWeight: 600, fontSize: 13, color: 'var(--earth)' }}>
                        {u.nama_user || '—'}
                      </div>
                    </div>
                  </div>
                </td>
                <td style={{ fontSize: 13, color: 'var(--warm-mid)' }}>{u.email}</td>
                <td>
                  <span className="badge" style={{
                    background: u.role === 'Admin' ? 'rgba(92,61,30,0.12)' : 'rgba(160,160,160,0.12)',
                    color: u.role === 'Admin' ? 'var(--earth)' : '#888',
                  }}>{u.role}</span>
                </td>
                <td>
                  <span className="user-subrole-badge" style={{
                    background: `${ROLE_COLORS[u.subrole] || '#888'}18`,
                    color: ROLE_COLORS[u.subrole] || '#888',
                  }}>{u.subrole}</span>
                </td>
                <td style={{ fontSize: 12, color: 'var(--warm-mid)', whiteSpace: 'nowrap' }}>
      ...
      {totalPages > 1 && (
        <div className="pagination">
          ...
          {Array.from({ length: Math.min(totalPages, 7) }, (_, i) => {
            ...
            return (
              <button key={pg} className={page === pg ? 'active' : ''} onClick={() => setPage(pg)}>
                {pg}
              </button>
            );
          })}
      ...
      {showCreate && (
        <CreateUserModal
          ...
          onSuccess={() => { setShowCreate(false); fetchList(1); toast.success('Akun berhasil dibuat.'); }}
          toast={toast}
        />
      )}
      ...
      {showEdit && editUser && (
        <EditUserModal
          ...
          onSuccess={() => { setShowEdit(false); fetchList(page); toast.success('Akun berhasil diperbarui.'); }}
          toast={toast}
        />
      )}
      ...
      {showKonten && kontenUser && (
        <div
          style={{
            ...
            zIndex: 150, display: 'flex', justifyContent: 'flex-end',
        ...
        >
          ...
          >
            ...
            <div style={{
              ...
              position: 'sticky', top: 0, background: 'var(--parchment)', zIndex: 10,
            }}>
              <div>
                <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--earth)', fontFamily: 'var(--font-display)' }}>
                  Konten oleh {kontenUser.nama_user}
                </div>
                <div style={{ fontSize: 11, color: 'var(--warm-mid)', marginTop: 2 }}>
                  {kontenUser.email} · {kontenUser.subrole}
                </div>
              </div>
              <button className="user-modal-close" onClick={() => setShowKonten(false)}>✕</button>
            </div>

            {/* Body drawer */}
            <div style={{ padding: 24 }}>
              {loadKonten ? (
                <div style={{ color: 'var(--warm-mid)', fontSize: 13 }}>Memuat konten…</div>
              ) : kontenList.length === 0 ? (
                <div className="empty-state">
                  <div className="empty-state-icon">📄</div>
                ...
                <>
                  <div style={{ fontSize: 12, color: 'var(--warm-mid)', marginBottom: 14 }}>
                    {kontenList.length} konten ditemukan
                  </div>
                  {kontenList.map((k) => (
                    <div key={k.id_konten} style={{
                      padding: '13px 0', borderBottom: '1px solid var(--border)',
                      display: 'flex', justifyContent: 'space-between',
                      alignItems: 'flex-start', gap: 12,
                    }}>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontWeight: 600, fontSize: 13, color: 'var(--earth)', marginBottom: 5 }}>
                          {truncate(k.judul, 52)}
                        </div>
                        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                          <span className="badge badge-berita" style={
                            k.jenis_konten === 'Edukasi' ? { background: 'rgba(139,111,71,0.12)', color: 'var(--bark)' } : {}
                          }>{k.jenis_konten}</span>
                          <span className={`badge badge-${k.status_validasi}`}>
                            {k.status_validasi === 'sudah' ? '✓' : '⏳'} {k.status_validasi}
                          </span>
                        </div>
                      </div>
                      <div style={{ fontSize: 11, color: 'var(--warm-mid)', whiteSpace: 'nowrap', flexShrink: 0 }}>
        ...
        <div className="user-modal-overlay">
          <div className="user-confirm-box">
            ...
            <div className="user-confirm-msg">
              ...
              <br />
              <span style={{ fontSize: 12, color: 'var(--warm-mid)' }}>{confirmDelete.email}</span>
              <br /><br />
              Tindakan ini tidak dapat dibatalkan. Semua konten yang dimiliki user ini tidak dapat diakses.
            </div>
...
const CreateUserModal = ({ onClose, onSuccess, toast }) => {
  const [form, setForm]     = useState({
    nama_user: '', email: '', password: '', role: 'User', subrole: 'User',
  });
  ...
  const handleChange = (e) => {
    ...
    setForm((f) => {
      ...
      if (name === 'role') next.subrole = value === 'User' ? 'User' : 'Jurnalis';
      return next;
    });
  };
        ...
        <form onSubmit={handleSubmit}>
          <div className="user-modal-body">
            ...
            <div style={{
              ...
              💡 Akun yang dibuat di sini langsung aktif dan bisa login ke sistem.
            </div>

            <div className="field-group">
              <label>Nama Lengkap *</label>
              <input className="field-input" name="nama_user" value={form.nama_user}
                onChange={handleChange} placeholder="Nama lengkap user" />
            </div>

            <div className="field-group">
              <label>Email *</label>
              <input className="field-input" name="email" type="email" value={form.email}
                onChange={handleChange} placeholder="email@parmalim.com" />
            </div>

            <div className="field-group">
              <label>Password * (min. 6 karakter)</label>
              <input className="field-input" name="password" type="password" value={form.password}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              ...
              <div className="field-group">
                <label>Subrole</label>
                <select
                  className="field-input" name="subrole" value={form.subrole}
                  onChange={handleChange} disabled={form.role === 'User'}
                >
                  {form.role === 'User'
                    ? <option value="User">User</option>
                    : ADMIN_ROLES.map((s) => <option key={s} value={s}>{s}</option>)
                  }
                </select>
              </div>
            </div>
            ...
            <div style={{
              ...
              <strong style={{ color: 'var(--earth)' }}>Ringkasan akses:</strong>
              <div style={{ marginTop: 6 }}>
                {form.role === 'User' && '• Bisa membaca konten dan berkomentar'}
                {form.subrole === 'Jurnalis'   && '• Bisa membuat dan mengelola konten sendiri'}
                {form.subrole === 'Validator'  && '• Bisa memvalidasi konten yang dikirim'}
                {form.subrole === 'Manajer'    && '• Bisa melihat insight dan moderasi komentar'}
                {form.subrole === 'Pengembang' && '• Akses penuh: semua fitur + kelola user'}
              </div>
            </div>
          </div>
...
const EditUserModal = ({ user, onClose, onSuccess, toast }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      const body = { nama_user: form.nama_user, email: form.email, role: form.role, subrole: form.subrole };
  ...
  return (
    <div className="user-modal-overlay">
      <div className="user-modal-box">
        ...
        {/* Info user */}
        <div style={{
          padding: '14px 24px', background: 'var(--parchment)',
          borderBottom: '1px solid var(--border)',
          display: 'flex', alignItems: 'center', gap: 12,
        }}>
          <div className="user-avatar-cell" style={{ width: 40, height: 40, fontSize: 14 }}>
            {getInitials(user.nama_user || user.email)}
          </div>
          <div>
            <div style={{ fontWeight: 700, fontSize: 14, color: 'var(--earth)' }}>{user.nama_user}</div>
            <div style={{ fontSize: 12, color: 'var(--warm-mid)' }}>Bergabung {formatDate(user.created_at)}</div>
          </div>
        </div>
